...
class Solver(Derivatives):
    ...
    def __init__(self,base,N=[35,32,32,3,1],L=[2,.875*2*math.pi,.6*2*math.pi],store=True):
        super(Solver,self).__init__(N,L)
        self.flag=0
        self.N=N#[Ny,Nx,Nz]

        self.L=L
        self.base=np.zeros((N[0],N[1],N[2],N[3]))
        self.base[:,:,:,0]=np.flip(np.repeat(np.repeat(base[:,np.newaxis,np.newaxis],N[1],axis=1),N[2],axis=2),0)
    ...
    def Step(self,u,P,BC,tau=True,scheme='Multi',inpy=0,outy=0,inpxz=0,outxz=0):

        if scheme=='Multi':
            self.style=''
            # Solve for the homogenous solutions
            temp=time.time()
            self.Dv,self.P1c,self.P2c,self.v1c,self.v2c=self.compl_store(BC,self.lam)
            ...
            if tau==True:
                ...
                temp=time.time()
                P0,v0=self.Pv(np.zeros((self.N[0],self.N[1],self.N[2]),dtype=np.complex128),self.ck,self.A0BC,self.lam,0,0,0,0)
                self.A0t+=time.time()-temp
                # Use the A0 and A1 problems to solve for P and v
                temp=time.time()
                P,v=self.tau_cor(P0,v0,P1,v1,S[:,:,:,1],self.lam)
            ...
            for i in range(3):
                # Solve for the homogenous solutions
                self.Dv,self.P1c,self.P2c,self.v1c,self.v2c=self.compl_store(BC,self.lam_init[i])
                # Compute S and Sp
