...
class Derivatives:
    ...
    def lap(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        # add curlu    
    def curlu(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        # Compute partial derivatives for curl
        # a[:,:,:,0:1,:] is u, a[:,:,:,1:2,:] is v, a[:,:,:,2:3,:] is w
        dwdy = self.dy(a[:,:,:,2:3,:], inpy, outy, inpxz, outxz)  # ∂w/∂y
        dvdz = self.dz(a[:,:,:,1:2,:], inpy, outy, inpxz, outxz)  # ∂v/∂z
        dudz = self.dz(a[:,:,:,0:1,:], inpy, outy, inpxz, outxz)  # ∂u/∂z
        dwdx = self.dx(a[:,:,:,2:3,:], inpy, outy, inpxz, outxz)  # ∂w/∂x
        dvdx = self.dx(a[:,:,:,1:2,:], inpy, outy, inpxz, outxz)  # ∂v/∂x
        dudy = self.dy(a[:,:,:,0:1,:], inpy, outy, inpxz, outxz)  # ∂u/∂y
        
        # Compute curl components: (∂w/∂y - ∂v/∂z, ∂u/∂z - ∂w/∂x, ∂v/∂x - ∂u/∂y)
        curl_x = dwdy - dvdz
        curl_y = dudz - dwdx
        curl_z = dvdx - dudy
        
        # Concatenate to form the curl vector field
        curlu = np.concatenate((curl_x, curl_y, curl_z), axis=-2)
